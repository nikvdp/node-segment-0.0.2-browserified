...
Segment.prototype.loadDict = function (name, type, convert_to_lower) {
  ...
  // } else {
    ...
    // hardcode dict paths so browserify can inline them
    var data;
    // console.log(filename)
    if (name == 'dict.txt')
        data = fs.readFileSync(__dirname + "/../dicts/dict.txt", 'utf8')
    if (name == 'dict2.txt')
        data = fs.readFileSync(__dirname + "/../dicts/dict2.txt", 'utf8')
    if (name == 'names.txt')
        data = fs.readFileSync(__dirname + "/../dicts/names.txt", 'utf8')
    if (name == 'wildcard.txt')
        data = fs.readFileSync(__dirname + "/../dicts/wildcard.txt", 'utf8')
...
Segment.prototype.useDefault = function () {
    ...
    // 强制分割类单词识别
    var modules = [
        require('./module/URLTokenizer'),            // URL识别
        require('./module/WildcardTokenizer'),       // 通配符，必须在标点符号识别之前
        require('./module/PunctuationTokenizer'),    // 标点符号识别
        require('./module/ForeignTokenizer'),        // 外文字符、数字识别，必须在标点符号识别之后
        // 中文单词识别
        require('./module/DictTokenizer'),           // 词典识别
        require('./module/ChsNameTokenizer'),        // 人名识别，建议在词典识别之后

        // 优化模块
        require('./module/EmailOptimizer'),          // 邮箱地址识别
        require('./module/ChsNameOptimizer'),        // 人名识别优化
        require('./module/DictOptimizer'),           // 词典识别优化
        require('./module/DatetimeOptimizer'),       // 日期时间识别优化
    ];
